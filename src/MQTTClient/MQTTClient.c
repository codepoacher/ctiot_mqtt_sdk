...
static int decodePacket(MQTTClient* c, int* value, int timeout)
{
    ...
    *value = 0;
    do
    {
        ...
        rc = c->ipstack->mqttread(c->ipstack, &i, 1, timeout,__FILE__,__LINE__);
        if (rc != 1)
            goto exit;
...
static int readPacket(MQTTClient* c, Timer* timer)
{
    ...
    int rc = c->ipstack->mqttread(c->ipstack, c->readbuf, 1, TimerLeftMS(timer),__FILE__,__LINE__);
    if (rc != 1)
    {
        goto exit;
    }
    ...
    if (rem_len > 0 && (rc = c->ipstack->mqttread(c->ipstack, c->readbuf + len, rem_len, TimerLeftMS(timer),__FILE__,__LINE__) != rem_len)) {
        rc = 0;
        goto exit;
    }
...
int keepalive(MQTTClient* c)
{
    ...
    if(c->ping_outstanding == 0)
    {
        ...
        {
            TimerInit(&wait_ping_rsp_timer);
            TimerCountdownMS(&wait_ping_rsp_timer, c->command_timeout_ms);
    ...
    else if(TimerIsExpired(&wait_ping_rsp_timer))
    {
        c->reping_count++;
